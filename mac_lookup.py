...
from config import BASE_APPLE_URL
...
def verify_serial(serial):
    ...
    """
    if len(serial) > 12:
        raise AttributeError("Serial number contains more than 12 characters.")
    if len(serial) < 4:
        raise AttributeError(
            "Serial number must contain at least the last 4 characters."
        )
    last_4 = serial[-4:]
...
def get_model(serial):
    ...
    for error in xml.iter("error"):
        if error.text:
            raise AttributeError("Serial number lookup error.")
